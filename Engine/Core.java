...
public class Core {
	...
	public void resize_poulp_by_half() {
		CollisionBox cb = poulp.getCollisionBox();
		CollisionBox half = new CollisionBox((int) (cb.getX()+(0.25*cb.getWidth())), (int) (cb.getY()+(0.25*cb.getHeight())), cb.getWidth()/2, cb.getHeight()/2);
		poulp.setCollisionBox(half);
	}

	public void reset_poulp_size(){
		if(poulp.getCollisionBox()==null)
			return;
		CollisionBox cb = poulp.getCollisionBox();
		CollisionBox up = new CollisionBox((int) (cb.getX()-(0.25*cb.getWidth()*2)), (int) (cb.getY()-(0.25*cb.getHeight()*2)), cb.getWidth()*2, cb.getHeight()*2);
		poulp.setCollisionBox(up);
	}
